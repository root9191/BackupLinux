...
# --- Basis-Verzeichnisse ---
BASE_BACKUP_DIR="/mnt/Daten/lichti/btrfs"
...
cp_with_error_handling() {
    ...
    if [ ! -e "$source" ]; then
        log_info "Überspringe nicht existierende Datei/Verzeichnis: $source"
        return 0
    fi
    ...
    if ! error_output=$(cp "$@" 2>&1); then
        log_error "Fehler beim Kopieren von $source nach $dest: $error_output"
        return 1
    fi
    
    # Setze Berechtigungen
    if [ -f "$dest" ]; then
        chmod --reference="$source" "$dest" 2>/dev/null || chmod 644 "$dest"
    elif [ -d "$dest" ]; then
        chmod --reference="$source" "$dest" 2>/dev/null || chmod 755 "$dest"
    fi
...
# Zusätzliche Hilfsfunktion für rsync
rsync_with_error_handling() {
    ...
    if [ ! -e "$source" ]; then
        log_info "Überspringe nicht existierendes Verzeichnis: $source"
        return 0
    }
    ...
    # Führe rsync aus
    if ! error_output=$(rsync -av --progress $options "$source/" "$dest/" 2>&1); then
        log_error "Fehler bei rsync von $source nach $dest: $error_output"
        return 1
    fi
    
    # Setze Besitzer für das Zielverzeichnis
    chown -R "$SUDO_USER:$SUDO_USER" "$dest"
...
backup_system_configs() {
    ...
    for dir in "${SYSTEM_CONFIG_DIRS[@]}"; do
        if [ -d "/etc/$dir" ]; then
            mkdir -p "${BACKUP_SUBDIRS[etc]}/$dir"
    ...
    if [ -d "/etc/NetworkManager/system-connections" ]; then
        ...
        mkdir -p "${BACKUP_SUBDIRS[etc]}/NetworkManager_system-connections"
        cp_with_error_handling -r "/etc/NetworkManager/system-connections/." \
            "${BACKUP_SUBDIRS[etc]}/NetworkManager_system-connections/"
    fi
}

backup_user_configs() {
    log_info "Sichere Benutzer-Konfigurationen..."
    
    # Dotfiles
    log_info "Sichere Dotfiles..."
    for file in "${DOTFILES[@]}"; do
        if [ -e "${USER_HOME}/${file}" ]; then
            cp_with_error_handling -a "${USER_HOME}/${file}" "${BACKUP_SUBDIRS[home]}/"
        fi
    done

    # Home-Verzeichnisse
    log_info "Sichere Home-Verzeichnisse..."
    for dir in "${HOME_DIRS[@]}"; do
        if [ -d "${USER_HOME}/${dir}" ]; then
            mkdir -p "${BACKUP_SUBDIRS[home]}/${dir}"
            cp_with_error_handling -r "${USER_HOME}/${dir}/." "${BACKUP_SUBDIRS[home]}/${dir}/"
        fi
    done
}

backup_application_data() {
    log_info "Sichere Anwendungsdaten..."
    
    # .cache Ordner
    log_info "Sichere .cache Ordner..."
    mkdir -p "${BACKUP_SUBDIRS[cache]}"
    local cache_excludes=$(create_rsync_excludes "${CACHE_EXCLUDES[@]}")
    backup_with_rsync "${USER_HOME}/.cache" "${BACKUP_SUBDIRS[cache]}" "$cache_excludes"

    # .config Ordner
    log_info "Sichere .config Ordner..."
    mkdir -p "${BACKUP_SUBDIRS[config]}"
    backup_with_rsync "${USER_HOME}/.config" "${BACKUP_SUBDIRS[config]}"

    # .local Ordner
    log_info "Sichere .local Ordner..."
    mkdir -p "${BACKUP_SUBDIRS[local]}"
    local local_excludes=$(create_rsync_excludes "${LOCAL_EXCLUDES[@]}")
    backup_with_rsync "${USER_HOME}/.local" "${BACKUP_SUBDIRS[local]}" "$local_excludes"

    # Flatpak-Daten
    if command -v flatpak &> /dev/null; then
        log_info "Sichere Flatpak-Einstellungen und Daten..."
        mkdir -p "$BASE_BACKUP_DIR/flatpak_data"
        mkdir -p "$BASE_BACKUP_DIR/flatpak_apps"
...
backup_themes_and_icons() {
    log_info "Sichere Themes und Icons..."
    ...
    if [ -d "/usr/share/themes" ]; then
        ...
        for theme in "${THEMES[@]}"; do
            if [ -d "/usr/share/themes/$theme" ]; then
                cp_with_error_handling -r "/usr/share/themes/$theme" "${BACKUP_SUBDIRS[usr_share]}/themes/"
            fi
        done
    fi
}
